...
Vue.component("data_form", {
        name: "data_form",
        props: ["current_data", "current_name", "current_type", "stack", "dev"],
        delimiters: ["[[", "]]"],
        data: function () {
            return {
        ...
        <div v-if="get_data_type(current_data) == 'Object' || get_data_type(current_data) == 'Array'">
            <table class="layui-table" lay-size="sm">
                ...
                <tr v-for="(value,key) in current_data">
                    <td>[[ key ]]</td>
                    <td><data_form :current_data=value :current_name="key" :current_type="current_type" :stack="add_item_to_array(stack, key)"
                                   @data_change="$emit('data_change', $event);" @delete_data="$emit('delete_data', $event)" :dev="dev"></data_form></td>
                    <td v-if="dev">
                        <div class="layui-btn-group">
                            <button class="layui-btn layui-btn-sm"><i class="layui-icon" title="新增"></i></button>
                            <button class="layui-btn layui-btn-danger layui-btn-sm"><i class="layui-icon" @click="$emit('delete_data', {stack:stack, index:key})" title="删除"></i></button>
                        </div>
                    </td>
                </tr>
                </tbody>
